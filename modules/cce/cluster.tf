...
resource "opentelekomcloud_vpc_eip_v1" "cce_eip" {
  count = local.cluster_config.public_cluster ? 1 : 0
  bandwidth {
    charge_mode = "traffic"
...
resource "opentelekomcloud_cce_cluster_v3" "cluster" {
  ...
  eip                     = local.cluster_config.public_cluster ? opentelekomcloud_vpc_eip_v1.cce_eip[0].publicip[0].ip_address : null
...
resource "opentelekomcloud_cce_node_pool_v3" "cluster_node_pool" {
  ...
  root_volume {
    size       = 50
    volumetype = "SSD"
    kms_id     = local.node_storage_encryption_enabled ? opentelekomcloud_kms_key_v1.node_storage_encryption_key[0].id : null
  }
