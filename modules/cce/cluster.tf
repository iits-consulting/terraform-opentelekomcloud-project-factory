...
resource "random_id" "id" {
  count       = var.node_config.node_storage_encryption_enabled ? 1 : 0
  byte_length = 4
}
...
resource "opentelekomcloud_cce_node_pool_v3" "cluster_node_pool" {
  count              = length(var.node_config.availability_zones)
  cluster_id         = opentelekomcloud_cce_cluster_v3.cluster.id
  name               = "${var.name}-nodes-${var.node_config.availability_zones[count.index]}"
  flavor             = var.node_config.node_flavor
  initial_node_count = var.node_config.node_count
  availability_zone  = var.node_config.availability_zones[count.index]
  ...
  user_tags                = var.tags
  docker_base_size         = 20
  postinstall              = var.node_config.node_postinstall

  root_volume {
    size       = 50
    volumetype = "SSD"
    kms_id     = var.node_config.node_storage_encryption_enabled ? opentelekomcloud_kms_key_v1.node_storage_encryption_key[0].id : null
  }

  data_volumes {
    size       = var.node_config.node_storage_size
    volumetype = var.node_config.node_storage_type
    kms_id     = var.node_config.node_storage_encryption_enabled ? opentelekomcloud_kms_key_v1.node_storage_encryption_key[0].id : null
  }
